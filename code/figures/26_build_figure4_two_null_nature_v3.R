...
if (length(args) != 1L) stop("Usage: Rscript 26_build_figure4_two_null_nature_v3.R <rair_audit_dir>")
...
canonical_gse151 <- file.path(
  project_dir, "covariance_null_v2", "clinical_label_layer_v1", "tables",
  "clinical_endpoint_two_null_results.tsv"
)
out_dir <- file.path(project_dir, "figures", "figure4_two_null_v4_unified_bootstrap")
...
gse151179 <- read_tsv(canonical_gse151, show_col_types = FALSE)
...
palette <- c(
  ...
  "Generic control" = "#858A91", GSE151179_uptake = "#4C78A8",
  GSE299988_RAI_LN = "#C65353"
)
...
clean_df <- gse151179 %>%
  ...
  transmute(
    signature_id, program = unname(program_labels[signature_id]),
    dataset_key = "GSE151179_uptake", dataset_label = "GSE151179 uptake axis",
    estimate = adverse_aligned_hedges_g,
    lower = bootstrap_ci_low, upper = bootstrap_ci_high
  )
challenge_primary <- two_null %>%
  filter(signature_id %in% primary_ids) %>%
  transmute(signature_id, program = unname(program_labels[signature_id]),
            dataset_key = "GSE299988_RAI_LN", dataset_label = "GSE299988 RAI/LN challenge",
            estimate = adverse_aligned_hedges_g,
            lower = frozen_bootstrap_ci_low, upper = frozen_bootstrap_ci_high)
shift_df <- bind_rows(clean_df, challenge_primary) %>%
  mutate(dataset_key = factor(dataset_key, levels = c("GSE151179_uptake", "GSE299988_RAI_LN")),
         program = factor(program, levels = c("TDS-16", "Iodide handling-11", "Condello-6")))
p_d <- ggplot(shift_df, aes(dataset_key, estimate, group = program)) +
  geom_hline(yintercept = 0, linetype = "dashed", linewidth = 0.35, colour = "#777777") +
  geom_line(colour = "#A8A8A8", linewidth = 0.55) +
  geom_errorbar(aes(ymin = lower, ymax = upper, colour = dataset_key), width = 0.055, linewidth = 0.5) +
  geom_point(aes(colour = dataset_key), size = 2.1) +
  ...
  scale_colour_manual(values = c(GSE151179_uptake = "#4C78A8", GSE299988_RAI_LN = "#C65353"),
                      guide = "none") +
  scale_x_discrete(labels = c(
    GSE151179_uptake = "GSE151179\nuptake",
    GSE299988_RAI_LN = "GSE299988\nRAI/LN"
  )) +
...
base <- file.path(out_dir, "Figure4_GSE299988_two_null_challenge_v4")
...
message("Figure 4 v4 written to: ", base)
